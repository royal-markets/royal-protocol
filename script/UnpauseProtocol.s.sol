...
contract UnpauseProtocol is Script {
    // =============================================================
    //                          INPUTS
    // =============================================================
    ...
    function run() external {
        ...
        if (idRegistry.paused()) {
            idRegistry.unpause();
            console.log("Unpaused IdRegistry");
        }

        if (idGateway.paused()) {
            idGateway.unpause();
            console.log("Unpaused IdGateway");
        }

        if (provenanceRegistry.paused()) {
            provenanceRegistry.unpause();
            console.log("Unpaused ProvenanceRegistry");
        }

        if (provenanceGateway.paused()) {
            provenanceGateway.unpause();
            console.log("Unpaused ProvenanceGateway");
        }
